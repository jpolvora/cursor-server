...
## Acceptance Criteria
- [ ] `GET /tasks/:id/stream` returns HTTP `200` with header `Content-Type: text/event-stream`.
- [ ] Streams `status` event on connection and when task status changes (`queued` -> `running` -> `completed` | `failed`).
- [ ] Streams `output` event chunks as logs/agent progress lines arrive.
- [ ] Emits `done` event and closes connection when task finishes execution.
- [ ] Rejects unauthorized connection attempts when `SERVER_API_KEY` is set.
- [ ] Typecheck (`npm run typecheck`) and build (`npm run build`) pass.
