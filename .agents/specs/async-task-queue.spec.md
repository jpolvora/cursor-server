...
## Acceptance Criteria
- [x] `POST /tasks` with `{ async: true, ... }` returns HTTP `202 Accepted` with `{ taskId, status: "queued", repo, agent }`.
- [x] Task state transitions: `queued` ➔ `running` ➔ `completed` or `failed`.
- [x] `GET /tasks/:id` returns full task status, duration, error or result summary once finished.
- [x] Non-blocking execution prevents client timeouts.
