...
Agentic Code Review and human reviewers often comment **after** the first push. ship-pr waits before the first solve-pr act and between rounds so `fetch_threads.cjs` sees stable threads.
...
Everything else unchanged: branch sync, solve-pr steps 0–7, auto gates, commit message `fix(#<PR>): auto-fix issues from review threads [...]`, verify build/tests, Escalar stops the pipeline.
