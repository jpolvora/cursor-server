# US Delivery Workflow v8.1 — Diagrams

> **Nota de arquitetura (v8.1, pós-refactor "orquestrador puro"):** os Steps 1, 2, 3, 5, 6, 10 e 11 delegam seu conteúdo funcional a skills dedicadas e standalone — `plan-us`, `refine`, `plan-exec-dag`, `implement-plan`, `verify-sync-plan-us`, `integration-validation` (ver [`us-workflow.md`](us-workflow.md) § Step Instructions). O `state.md` funciona como memória local do workflow e o `MEMORY.md` como memória compartilhada/generalizável.

Visual docs for the [`us-workflow.md`](us-workflow.md) agent. Human guide: [`README.md`](README.md).
...
flowchart TB
    ...
    subgraph Orchestrator["Orchestrator (main agent)"]
        ...
        PROMPT["Build prompt from us-workflow.md"]
...
sequenceDiagram
    ...
    O->>O: Build prompt from us-workflow.md step section
...
flowchart TD
    subgraph S11["Step 11 — Integration validation"]
        ...
        BUILD["stack.md: npm run typecheck + npm run build"]
...
mindmap
  root((US Delivery<br/>Workflow v8.1))
    ...
    Step 1
      Plan template
      cursor-server layered architecture (routes / services / jobs)
    Step 2
      Refinement loop
      Decision log
    Step 3
...
| **us-workflow.md** | Single source of step instructions (v8.1) |
